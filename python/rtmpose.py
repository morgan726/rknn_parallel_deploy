...
def preprocess(
    ...
    cv2.imwrite("affine.jpg",resized_img)
    # normalize image
...
def postprocess(outputs: List[np.ndarray],
    ...
    # 写入simcc_x
    with open("simcc_x.txt", "w") as f:
        for i in range(simcc_x.shape[0]):
            for j in range(simcc_x.shape[1]):
                f.write(f"{simcc_x[i][j]} ")
            f.write("\n")

    # 写入simcc_y
    with open("simcc_y.txt", "w") as f:
        for i in range(simcc_y.shape[0]):
            for j in range(simcc_y.shape[1]):
                f.write(f"{simcc_y[i][j]} ")
            f.write("\n")
...
def visualize(img: np.ndarray,
    ...
    link_color = [
        1, 1, 2, 2, 0, 0, 0, 0, 1, 2, 1, 2, 0, 0, 0, 0, 0
    ]
    point_color = [
...
def onnx_main():
    ...
    # image_file = "./20250902-170623.jpg"
    # device = 'cpu'
    # onnx_file = '../model/rtmpose/rtmposem.onnx'
    # save_path = './out.jpg'
    image_file = "/home/dreame/dmt/data/nvme7/dataset/pose/strip/result/1_strip_0818.jpg"
    device = 'cpu'
    onnx_file = '/home/dreame/dmt/data/nvme7/code/toolbox/mm/rtmpose2onnx/end2end.onnx'
    save_path = './out.jpg'
    ...
    visualize(img, keypoints, scores, save_path)

    logger.info('Done...')


def rknn_infer(model,img):
    logger.info('Start running model on RTMPose...')
    # image_file = "/media/nvidia/D6612D9737620A9A/alg/rtm/python/human-pose.jpeg"
    # # rknn_path = '/media/nvidia/D6612D9737620A9A/alg/rtm/python/models/rtmposem_fp16.rknn'
    # rknn_path = '/media/nvidia/D6612D9737620A9A/alg/rtm/python/models/rtmposem.rknn'
    save_path = '/media/nvidia/D6612D9737620A9A/alg/rtm/python/out.jpg'
    # read image from file
    # logger.info('1. Read image from {}...'.format(image_file))
    # img = cv2.imread(image_file)

    # build onnx model
    # logger.info('2. Build onnx model from {}...'.format(rknn_path))
    ...
    logger.info('4. Inference...')
    start_time = time.time()
    outputs = model.run([resized_img.astype(np.float16)])
    end_time = time.time()
    logger.info('4. Inference done, time cost: {:.4f}s'.format(end_time -
                                                               start_time))
...
def rknn_main():
    ...
    logger.info('Start running model on RTMPose...')
    image_file = "/home/dreame/dmt/data/nvme7/dataset/pose/strip/result/1_strip_0818.jpg"
    rknn_path = '/home/dreame/dmt/data/nvme7/code/toolbox/mm/rtmpose2onnx/end2end.rknn'
    # rknn_path = '/home/dreame/dmt/program/project/rknn_yolox_nano-rtmpose/examples/aeke_pose_deploy/model/rk3562/strip.rknn'
    save_path = 'outr.jpg'
    # read image from file
    logger.info('1. Read image from {}...'.format(image_file))
    img = cv2.imread(image_file)
    ...
    logger.info('2. Build onnx model from {}...'.format(rknn_path))
    from rknn_executor import RKNN_model_container 
    model = RKNN_model_container(rknn_path,target='rk3562')
