...
static inline float IOU(float XMin1, float YMin1, float XMax1, float YMax1, float XMin2, float YMin2, float XMax2,
                        float YMax2) {
  float Inter = 0;
  float Total = 0;
  float XMin = 0;
...
static float DeQnt2F32(int8_t qnt, int zp, float scale) { return ((float)qnt - (float)zp) * scale; }
...
float GetResultRectyolov11::sigmoid(float x) { return 1 / (1 + fast_exp(-x)); }
...
int GetResultRectyolov11::GetConvDetectionResult(int8_t **pBlob, std::vector<int> &qnt_zp,
                                                 std::vector<float> &qnt_scale, std::vector<float> &DetectiontRects) {
  int ret = 0;
  if (meshgrid.empty()) {
  ...
  for (int index = 0; index < headNum; index++) {
    ...
    for (int h = 0; h < mapSize[index][0]; h++) {
      for (int w = 0; w < mapSize[index][1]; w++) {
        ...
        if (1 == class_num) {
          cls_max = sigmoid(DeQnt2F32(cls[0 * mapSize[index][0] * mapSize[index][1] + h * mapSize[index][1] + w],
        ...
        if (cls_max > objectThresh) {
          ...
          for (int lc = 0; lc < 4; lc++) {
            ...
            for (int df = 0; df < 16; df++) {
              locvaltemp = exp(
                  DeQnt2F32(reg[((lc * 16) + df) * mapSize[index][0] * mapSize[index][1] + h * mapSize[index][1] + w],
  ...
  std::sort(detectRects.begin(), detectRects.end(),
            [](DetectRect &Rect1, DetectRect &Rect2) -> bool { return (Rect1.score > Rect2.score); });
...
int rtmpose_postprocess(float *simcc_x_result, float *simcc_y_result, cv::Mat affine_transform_reverse,
  ...
  for (int i = 0; i < pose_result.size(); ++i) {
    ...
    std::cout << pose_result[i].x << "   " << pose_result[i].y << "   " << pose_result[i].score << std::endl;
  }
